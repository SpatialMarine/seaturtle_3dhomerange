...
# Create gif for 3D plot automatically
gif_plot <- TRUE
...
if (gif_plot) {
  ...
  rgl::movie3d(spin3d(axis = c(0, 0, 1), rpm = 2),
                 fps = 5,
                 duration = 60,
                 movie = "3d_95UD_LL_fishing_overlap_day_",
  ...
  # export
  magick::image_write(gif, paste0(output_dir,"/fig/3d_95UD_LL_fishing_overlap_day.gif"))
  # remove frames file
  file.remove(imgs)
}
