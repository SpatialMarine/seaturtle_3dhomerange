...
b <- terra::rast(paste0(input_dir,"/gis/gebco/mediterranean_sea_gebco_2024.tif"))
# for some incompatibilities with raster package
...
p <- ggplot() +
  ...
  geom_path(data = data,
            aes(x = longitude, y = latitude, group = interaction(organismID, trip)),
            size = 2.5, alpha = 0.22, colour = "#989ce3") +
  geom_path(data = data,
            aes(x = longitude, y = latitude, group = interaction(organismID, trip)),
...
# save plot
p_png <- paste0(output_dir, "/fig/location_map.png")
p_svg <- paste0(output_dir, "/fig/location_map.svg")
