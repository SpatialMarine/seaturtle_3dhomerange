...
# Create data frame to store results
  ...
  write.csv(kde_2d_res, paste0(kde_folder,"/",organismID,"_2d_res_day",".csv"), row.names = FALSE)
  ...
  save(ttdr, file = finalttdrfile)
  save(kde_2d_res, file = resfile)
  write.csv(kde_2d_res, paste0(kde_folder,"/",organismID,"_2d_res_night",".csv"), row.names = FALSE)
...
files <- files[grepl("/\\d+_2d_res_day\\.csv$", files)] # select only .csv with organismID in the name (for future changes)
# combine csv into single one
df <- files %>% 
  purrr::map_df(read.csv)
# save / export combined result for 3D kernel density estimation
write.csv(df, paste0(output_data,"/kde_2d_res_day.csv"), row.names = FALSE)


# list results for all individuals
files <- list.files(output_data, pattern = "_2d_res_night.csv", recursive = TRUE, full.names = TRUE)
files <- files[grepl("/\\d+_2d_res_night\\.csv$", files)] # select only .csv with organismID in the name (for future changes)
# combine csv into single one
df <- files %>% 
  purrr::map_df(read.csv)
# save / export combined result for 2d kernel density estimation
write.csv(df, paste0(output_data,"/kde_2d_res_night.csv"), row.names = FALSE)
