...
# 1) load / import data 
# 2) process 3D overlap between fishing effort data and day night 
# kde organism ID processed 

#   - 2.1 Load data for processing - kde and fishing effort by day and night
#   - 2.2 Calculate the 3D overlap
#   - 2.3 Calculate the 3D overlap volumes by day / night
...
# create ouput dir
output_data <- paste0(output_dir,"/03_fishing_3d_overlap_daynight")
...
# empty dataframe to add results of overlap
...
# Processing day and night overlap
...
for (dnp in 1:length(daynight_pattern)) {
  ...
  # note: volumes in m3
  kde_res <- read.csv(paste0(main_dir,"/output/01_kde_3d/kde_3d_res_",dn,".csv"))
  ...
  for (i in 1:length(kde_files)) {
    ...
    # note: volumes in km3, transformed previously
    ...
    for (g in 1:length(fishing_gears)) {
      ...
      # load fishing data
      fishing <- raster::stack(paste0(main_dir,"/output/03_fishing_3d_daynight/",organismID,"_3d_fishing-effort_",fishing_gear,"_",dn,".tif"))
      ...
      # logic: remove voxels where there are fishing impact in order to calculate the 
      # result: is the volume affects by fishing activities 
      ...
      # export raster files ------ 
      # 1) fishing interact
      rst_file <- paste0(output_data,"/",organismID,"_3d_kde_fishing_interact_",fishing_gear,"_",dn,".tif")
      writeRaster(fishing_interact, rst_file, overwrite=TRUE)
      
      # 2) kde fishing intersect
      rst_file <- paste0(output_data,"/",organismID,"_3d_kde_fishing_intersect_",fishing_gear,"_",dn,".tif")
      writeRaster(kde_fishing_intersect , rst_file, overwrite=TRUE)
      
      # 3) kde fishing difference
      rst_file <- paste0(output_data,"/",organismID,"_3d_kde_fishing_difference_",fishing_gear,"_",dn,".tif")
...
file <- paste0(output_data,"/3d_kde_fishing_overlap_results_daynight.csv")
