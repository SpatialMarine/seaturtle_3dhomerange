...
# create ouput dir
output_data <- paste0(output_dir,"/03_fishing_2d_overlap")
...
# note: areas in m2
kde_res <- read.csv(paste0(main_dir,"/output/02_kde_2d/kde_2d_res.csv"))
...
for (i in 1:length(kde_files)) {
  ...
  for (g in 1:length(fishing_gears)) {
    ...
    # load fishing data
    fishing <- raster(paste0(main_dir,"/output/03_fishing_2d/",organismID,"_2d_fishing-effort_",fishing_gear,".tif"))
    ...
    # export raster files ------ 
    # 1) fishing interact
    rst_file <- paste0(output_data,"/",organismID,"_2d_kde_fishing_interact_",fishing_gear,".tif")
    writeRaster(fishing_interact, rst_file, overwrite=TRUE)
    
    # 2) kde fishing intersect
    rst_file <- paste0(output_data,"/",organismID,"_2d_kde_fishing_intersect_",fishing_gear,".tif")
    writeRaster(kde_fishing_intersect , rst_file, overwrite=TRUE)
    
    # 3) kde fishing difference
    rst_file <- paste0(output_data,"/",organismID,"_2d_kde_fishing_difference_",fishing_gear,".tif")
...
file <- paste0(output_data,"/2d_kde_fishing_overlap_results.csv")
