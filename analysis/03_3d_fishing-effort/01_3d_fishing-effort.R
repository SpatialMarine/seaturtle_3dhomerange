...
cat("Drifting longline range determined (min - mazx; mean): ", overall_stats$final_range_mean) # 30 - 70 m




# -----------------------------------------------------------------------------
# 3D three-dimensional fishing effort
...
# ------------------------------------------------------------------------------
# 2.1) Fishinf effort and fishing gears features
...
for (g in 1:length(fishing_gears)) {
  ...
  for (i in 1:length(kde_files)) { 
    ...
    if (fishing_gear == "LL") {
      ...
      fsh_layer = (LL_fishing_depth/10) # spcify number of depth following fishing gear type set depth as number (e.i., 20m = layer 2, 40m == layer 4)
      ...
      rst_file <- paste0(output_data,"/",organismID,"_3d_fishing-effort_",fishing_gear,".tif")
