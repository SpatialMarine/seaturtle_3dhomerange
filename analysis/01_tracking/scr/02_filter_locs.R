...
input_data <- paste0(output_dir, "/tracking/", sp_code, "/L0_locations")
output_data <- paste0(output_dir, "/tracking/", sp_code, "/L1_locations")
...
foreach(i=tags, .packages=c("dplyr", "ggplot2", "gridExtra", "grid", "data.table", "argosfilter", "stringr", "SDLfilter")) %dopar% {
  ...
  # plot histo
  if(tag_type == "PTT") p2 <- diffTimeHisto(dataL1, vline=24)
