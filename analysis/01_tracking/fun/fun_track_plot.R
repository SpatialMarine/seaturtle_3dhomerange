...
diffTimeHisto <- function(data, vline=24){
  ...
  p <- ggplot(data = data, aes(x=timedif, y = ..count..)) +
    geom_histogram(breaks = c(0, 6, 12, 24, 48, 72, 168), fill ="#4271AE") + 
    scale_x_continuous(name = "Time difference (hours)",
                       breaks = c(0, 6, 12, 24, 48, 72, 96, 120, 144, 168),
                       limits=c(0, 168)) +
...
#-----------------------------------------------------------------
# mapBehaviour       Map locations by behaviour label
#-----------------------------------------------------------------
mapBehaviour <- function (data){
